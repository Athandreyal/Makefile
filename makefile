...
#	3) SHare and share alike.  It made its way to you, pass it along, show the first years when its time.  Lets make it unending.
...
#ver1
# 	author: prenw499, Phillip Renwick, Q1 2016
#	initial
#ver2
# 	author: prenw499, Phillip Renwick, Q1 2016
...
#ver 5
#	author: prenw499, Phillip Renwick, Q1 2017
...
#primary build rules
#all:CPPFLAGS:=$(CPPFLAGS) $(SPECIALFLAGS)
all:CPPFLAGS:=$(filter-out $(EXCLUDEFLAGS),$(CPPFLAGS) $(SPECIALFLAGS))
all:$(if $(TESTLAST),clean,mm)
all:$(MODULES)
	@rm -f .mt
...
test:CPPFLAGS:=$(filter-out $(EXCLUDEFLAGS),$(CPPFLAGS) $(GTFLG) $(LDFLG) $(SPECIALFLAGS))
test:$(if $(MAINLAST),clean,mt)
test:$(TMODULES)
	@rm -f .mm
